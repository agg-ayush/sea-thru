...
import time
import io
sys.path.append(os.path.join(os.path.dirname(__file__), "deps", "monodepth2"))
...
def run(args):
    ...
    img = Image.fromarray(rawpy.imread(args.input).postprocess()) if args.raw else pil.open(args.input).convert('RGB')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--no-cuda', action='store_true', help='If set, always use CPU')
    parser.add_argument('--input', required=True, help='Input image')
    parser.add_argument('--output', default=None, help='Output filename (default: output/{input_name}.png)')
    ...
    parser.add_argument('--size', type=int, default=2048, help='Size to output')
    ...
    if args.output is None:
        ...
        args.output = os.path.join(output_dir, f'{input_basename}.png')
